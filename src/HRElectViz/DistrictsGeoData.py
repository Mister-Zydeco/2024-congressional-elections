...
def transform_coords(coords, transformer):
    if isinstance(coords[0], (float, int)):  # Point
        ...
        return [transform_coords(sub_coords, transformer) for sub_coords in coords]
...
def filter_by_state_f(data: GeoJSONfcb, state_abbrs: list[str], exclude: bool) -> None:
    ...
    data["features"] = [feature for feature in data["features"] if condition(feature)]
