...
class HrElection:
    def __init__(self):
        state_fips_x = pl.col("State\nCode").replace(ush.abbr_to_fips)
        ...
        df = df.with_columns(
            pl.format("{}{}", state_fips_x, pl.col("fips_d_")).alias("GEOID"),
            pl.col("d_number_").cast(pl.Int8).alias("District\nNumber"),
        ).drop(["d_number_", "fips_d_"])
        self.dfs["states_and_territories"] = df
        self.dfs["states"] = df.filter(pl.col("State\nCode").is_in(ush.state_abbrs))
    ...
    def get_districts_ranked_by_vote(self) -> pl.DataFrame:
        df: pl.DataFrame = (
            ...
            .select(SD_COLS + ["GEOID", "Party", "Name", "Vote"])
        )
    ...
    def get_district_winners(self) -> pl.DataFrame:
        ...
        df: pl.DataFrame = (
            ...
            .first()
            .select(SD_COLS + ["GEOID", "Party", "Name"])
        )
    ...
    def get_district_major_party_vote(self) -> pl.DataFrame:
        ...
        df: pl.DataFrame = (
            ...
            .group_by(["State\nCode", "District\nNumber", "Party"], maintain_order=True)
            .sum()
